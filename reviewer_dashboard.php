...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_application'])) {
    ...
    $stmt = $conn->prepare("UPDATE applications SET username=?, email=?, contact=?, address=?, school=?, course=?, year_level=?, gpa=?, family_income=?, essay=?, status=? WHERE id=?");
    $stmt->bind_param("sssssssssssi", $username, $email, $contact, $address, $school, $course, $year_level, $gpa, $family_income, $essay, $status, $id);
                        ...
                        <td>
                            <div class="action-buttons">
                                ...
                                <?php if($row['status'] == 'Pending'): ?>
                                    <a href="?approve=<?php echo $row['id']; ?><?php echo $search ? '&search='.urlencode($search) : ''; ?><?php echo $statusFilter ? '&status='.urlencode($statusFilter) : ''; ?><?php echo $page > 1 ? '&page='.$page : ''; ?>" class="action-btn btn-approve" onclick="return confirm('Approve this application?')">Approve</a>
                                    <a href="?reject=<?php echo $row['id']; ?><?php echo $search ? '&search='.urlencode($search) : ''; ?><?php echo $statusFilter ? '&status='.urlencode($statusFilter) : ''; ?><?php echo $page > 1 ? '&page='.$page : ''; ?>" class="action-btn btn-reject" onclick="return confirm('Reject this application?')">Reject</a>
                                <?php endif; ?>
                                ...
                                <button class="action-btn btn-edit" onclick="openEditModal(
                                    ...
                                    '<?php echo addslashes($row['gpa']); ?>',
                                    '<?php echo addslashes($row['family_income']); ?>',
        ...
        <div class="modal-body">
            <form class="edit-form" method="POST" action="">
                ...
                <div class="detail-grid">
                    ...
                    <div class="form-group">
                        <label>GPA</label>
                        <input type="text" name="gpa" id="edit-gpa">
                    </div>
                    ...
                    <div class="form-group">
                        <label>School</label>
                        <input type="text" name="school" id="edit-school" required>
                    </div>
                    <div class="form-group">
                        <label>Course</label>
                    ...
                    <div class="form-group">
                        <label>Year Level</label>
                        <select name="year_level" id="edit-year">
                    ...
                    <div class="form-group">
                        <label>Family Income</label>
                        <select name="family_income" id="edit-income">
...
<script>
    ...
    function openEditModal(id, username, email, contact, address, school, course, year, gpa, income, essay, status) {
        ...
        document.getElementById('edit-income').value = income || 'Below 100,000';
        document.getElementById('edit-essay').value = essay || '';
